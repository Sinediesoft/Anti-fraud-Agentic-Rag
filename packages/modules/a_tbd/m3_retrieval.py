...
def build_index(*, batch: int = EMBED_BATCH, checkpoint_every: int = CHECKPOINT_EVERY) -> int:
    ...
    **斷點續建**：每 checkpoint_every 筆把編好的部分存進 _build_vectors.npy，
    ...
    2026-09-24 加的。語料從 17,764 筆改成 81,423 筆之後要連編好幾個小時，
    中途掉電從頭來過的代價太高。

    速度別照抄別人的：shared/models.py 的檔頭記著 1.17 筆/秒（照那個算
    81,423 筆要 19 小時），但同一天在 A 這台（M5／Metal、fp32）實測是
    9.15 筆/秒、約 2.5 小時，差了快 8 倍。要排程就自己量一次。
