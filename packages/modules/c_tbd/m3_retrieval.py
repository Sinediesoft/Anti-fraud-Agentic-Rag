...
# ⚠ 暫定值，量出來的，但樣本很小 —— S12 要在完整索引＋標註評測集上重量。
#
# 2026-09-20 在 demo 索引（前 300 筆真實語料、bge-m3、cpu/fp32）上實測：
#
#     相關問句「我在臉書看到投資廣告，加了LINE群組後匯款」
#         最高 0.8266 / 前 10% 0.7649 / 中位 0.6976 / 最低 0.5863
#     弱相關「對方叫我去超商買遊戲點數然後拍序號」
#         最高 0.6194 / 第二名 0.5590
#     離題「我明天要去菜市場買水果」
#         最高 0.4653   ← 門檻要落在它上面
#
# 所以可用的窗口是 0.4653 ~ 0.5590，中間取 0.50。
#
# 🔴 這個窗口只有 0.09 寬，而且是三個問句、300 筆語料量出來的。餘裕很小，
#    換切塊策略或把索引建到 10,051 筆都可能讓它失效。在 S12 的標註評測集
#    出來之前，把它當「比 0.0 好」而不是「這就是對的值」。
#
