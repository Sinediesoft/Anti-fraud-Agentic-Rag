...
SHOPPING = Profile(
    ...
    stages=(
        ...
        Stage(
            8,
            "匯款付款",
            # 補了三種 gold 有、原 pattern 沒抓到的付款形式：買點數、面交、只寫金額
            r"匯款|匯了|轉帳|付款|匯入|支付|轉了|匯過去|繳款|儲值"
            r"|購買.{0,6}(點數|禮券|禮卷|遊戲卡)"
            r"|(面交|當面交易|約.{0,4}見面).{0,10}(購買|付|買|交易)"
            r"|(共計|總計|合計).{0,6}[0-9０-９]"
            # 「被騙 N 元」也算付款：案主講得出金額，錢就已經出去了。
            # v4 gold 修正（2026-09-24）——原本只認「損失」加**阿拉伯數字**，
            # 漏掉兩種常見寫法：換個動詞（「被詐騙 5,000 元」），
            # 以及中文數量詞（「被騙五千」「被詐騙數千元」）。母體實測 +85 筆（2.1%）。
            # 單獨的「被騙」不算 —— 沒有金額時可能是被騙個資、或只是通報遇過這個手法，
            # 那仍屬尚未付款（語料裡確實有這種案例，不是所有報案人都已受害）。
            r"|(被騙|被詐騙|詐騙|遭騙|受騙|損失)[^。，]{0,10}"
            r"(?:[0-9０-９][0-9０-９,，]*|[一二三四五六七八九十百千萬兩數幾上]+)\s*[元萬塊]",
            0.742,
        ),
        ...
        Stage(
            ...
            # 「無卡提款」單獨就算——一般網購不會要你開這個功能，
            # 而 gold 裡有 4 筆寫「設定無卡提款」卻沒提 ATM，原 pattern 全漏。
            r"|無卡提款|付款碼|提款密碼"
            r"|(ATM|提款機|自動櫃員機)[^。]{0,14}(設定|無卡|解除|英文|轉換|按鍵)"
            r"|(網銀|網路銀行|網路ATM)[^。]{0,10}(驗證|認證|解除|綁定|設定)"
            r"|視訊.{0,6}(操作|認證|驗證)|螢幕.{0,4}(共享|分享)"
            r"|提供.{0,6}(金融卡|提款卡|信用卡).{0,4}密碼",
            0.292,
        ),
        ...
        Stage(
            ...
            # 第三版：gold 裡有 7 筆漏抓，補上四種寫法——
            # 「盜走」（不帶帳戶主詞）、「錢不見」、「遭無卡提款」（原本只寫提領）、「人頭帳戶」。
            r"(存款|帳戶|款項|餘額|存簿|戶頭|卡).{0,8}(被|遭)[^。，]{0,8}"
            r"(轉出|盜領|盜刷|扣|提領|提款|領走|轉走|領出)"
            r"|(被|遭).{0,3}(盜刷|盜領|盜轉|盜用|盜走)|盜走"
            r"|(錢|款項|存款|餘額).{0,6}(不見|消失|少了)"
...
# 「又匯」「再轉一次」「陸續匯了」這類標記才代表付了第二次。
# 原本是數付款詞出現幾次，但「我匯款…匯款後…」是同一筆被描述兩次，
# gold 裡有 7 筆因此被誤判成 repeated。
_REPEAT_MARK = re.compile(
    # 「再／又」只是其中一種。gold 裡更常見的是「前後一共匯了 4 筆」「第二筆匯款」
    # 「第一筆是⋯」（暗示還有第二筆）、「不斷要求更多的匯款」。
    r"(再|又|二度|第二次|第三次|另外|追加|陸續|多次|數次|好幾次|繼續|不斷|一連串|持續)"
    r"[^。]{0,10}(匯|轉帳|轉了|付|繳|存入|儲值|加碼)"
    r"|(匯|轉|付)[^。]{0,6}(第二|第三|兩次|三次|多次|數次)"
    r"|(一共|總共|前後|合計|累計)[^。]{0,8}(匯|轉|付|筆)"
    r"|第[一二三四五六七八九十]筆"
    r"|[0-9０-９]+[ ]*筆[^。]{0,8}(匯|轉|付|款)"
    r"|更多[^。]{0,6}(匯款|款項|資金|錢|金額)"
)


def _count(profile: Profile, text: str) -> int:
    """回傳 1 表示付過款、2 表示有重複付款的跡象、0 表示沒付款。

    不再數付款詞的出現次數——那數的是「這件事被提到幾次」，不是「付了幾次」。
    """
    stage = next(s for s in profile.stages if s.no == profile.pay_stage)
    if not re.search(stage.pattern, text):
        return 0
    return 2 if _REPEAT_MARK.search(text) else 1
