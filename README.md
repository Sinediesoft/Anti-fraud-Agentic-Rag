...
| | 題目 | 進度 | 入場檢查（七項） |
|---|---|---|---|
| A | LINE × 假投資 | 語料 81,423 筆（只篩平台、不篩標籤）、向量索引重建中、M4 規則層已可跑 | **7/7 可掛載**（考題 19/20，待用新語料重量） |
| B | **未宣告** | 只有 README | — |
...
packages/modules/
  ...
  b_tbd/ … e_tbd/        其他四人的佔位
tools/                   機器人：越界檢查、修改範圍檢查；主要架構圖的產生器
tests/                   規矩全部寫成測試
docs/                    主要架構、模型版本表、向量庫選型、GitHub 速查、怎麼新增第六個模組
  廢案/                  已作廢的舊圖，只保存、不更新
data/                    語料放這裡（不進版控）
```
...
- **[專題主要架構](docs/architecture.md)** ← 產品流程圖：每一步呼叫的模型、跑 CPU 還是 GPU、對應哪個程式碼檔案；網頁版與怎麼改都寫在裡面
- [進度同步](docs/進度同步/) ← 全隊要一起對齊的進度文件
- [廢案](docs/廢案/) ← 已作廢的舊流程圖，只保存、不更新
